...
export default function QuestionCompleteSentence({ sentence, options, correctIndex, onAnswer }) {
  ...
  return (
    <div className={styles.question}>
      <p className={styles.statement}>{sentence}</p>
      <ul className={styles.optionsList}>
        {options?.map((opt, i) => (
          <li key={i}>
            <button
              type="button"
              className={`${styles.option} ${styles.completeOption} ${selected === i ? styles.selected : ""} ${getOptionClass(i)}`}
